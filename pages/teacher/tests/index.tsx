                    ...
                    <div style={{ display: 'grid' }}>
                        {quiz.map((singleQuiz: any) => (
                            <Box
                                ...
                                onClick={() =>
                                    router.push(`/quiz/${singleQuiz.id}`)
                                }
                            >
...
export async function getServerSideProps() {
    try {
        const quiz = await fetch(BASE_URL + api.getQuiz).then((res) =>
            res.json()
        );
        const users = await fetch(BASE_URL + api.getUser).then((res) =>
            res.json()
        );
        const data = quiz.map((singleQuiz: any) => {
            return {
                ...singleQuiz,
                user: users?.find(
                    (user: any) => user.uid === singleQuiz.userId
                ),
            };
        });
